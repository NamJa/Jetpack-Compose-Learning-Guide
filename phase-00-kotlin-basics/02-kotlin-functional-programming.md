...
// 다양한 호출 방식
createUser("홍길동")                      // User(홍길동, 0, )
createUser("홍길동", 25)                  // User(홍길동, 25, )
createUser("홍길동", 25, "hong@mail.com") // User(홍길동, 25, hong@mail.com)
```
...
// any / all / none: 조건 검사
numbers.any { it > 5 }    // true  (하나라도 5보다 큰가?)
numbers.all { it > 0 }    // true  (모두 0보다 큰가?)
numbers.none { it < 0 }   // true  (0보다 작은 게 없는가?)
```
